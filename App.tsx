...
// Mock data for B/C side simulation
const MOCK_PREFILLED_CUSTOMER = {
  firstName: 'John',
  lastName: 'Citizen',
...
const MOCK_PREFILLED_INSTALLER = 'SolarHub'; // Example for B-side

type AppMode = 'public' | 'b' | 'c';
type InstallationStatus = 'installed' | 'considering' | 'signed' | 'rejected' | null;
...
export default function App() {
  ...
  const [installStatus, setInstallStatus] = useState<InstallationStatus>(null);

  useEffect(() => {
    // Detect mode from URL query param "?type="
    // type=b -> B-side (Installer fixed/readonly, Customer hidden)
    // type=c -> C-side (Installer selectable, Customer pre-filled & hidden, Quote btn hidden)
    ...
    if (type === 'b') {
      ...
      setFormData(prev => ({
        ...prev,
        installerName: MOCK_PREFILLED_INSTALLER,
        customer: MOCK_PREFILLED_CUSTOMER
      }));
    } else if (type === 'c') {
      setMode('c');
      setFormData(prev => ({
  ...
  const handleStatusChange = (status: InstallationStatus) => {
    setInstallStatus(status);
    // Logic: If installed or signed, it is NOT quote only. 
    // If considering or rejected, it IS quote only.
    const isQuoteOnly = status !== 'installed' && status !== 'signed';
    setFormData(prev => ({
      ...prev,
      isQuoteOnly
    }));

    // Optional: Scroll to next section after short delay
    if (status) {
      setTimeout(() => {
        const nextSection = document.getElementById('main-form-content');
        if (nextSection) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (validateForm()) {
      ...
      console.log('Submitting:', { ...formData, installStatus });
      setSubmitted(true);
      window.scrollTo({ top: 0, behavior: 'smooth' });
    } else {
      // Scroll to the first error
      const errorFieldId = errors.installerName ? 'installer-section' : 
                           'customer-details';
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
        <div className="bg-white p-8 rounded-2xl shadow-xl max-w-md w-full text-center">
          ...
          <button
            onClick={() => {
              // Reset but keep mode logic
              const currentCustomer = mode === 'public' ? INITIAL_STATE.customer : MOCK_PREFILLED_CUSTOMER;
              const currentInstaller = mode === 'b' ? MOCK_PREFILLED_INSTALLER : '';
              
              setFormData({
                ...INITIAL_STATE,
                installerName: currentInstaller,
  ...
  return (
    <div className="min-h-screen bg-gray-50 font-sans text-gray-900 pb-24">
        ...
        <div className="max-w-4xl mx-auto px-4 py-4 flex items-center gap-3">
          ...
          {mode !== 'public' && (
             <span className="ml-auto text-xs bg-gray-100 text-gray-500 px-2 py-1 rounded">
               {mode === 'b' ? 'Verified Client' : 'Partner Link'}
             </span>
          )}
        </div>
      </header>
      ...
      <main className="max-w-4xl mx-auto px-4 py-8 space-y-8">
        ...
        {/* B/C Mode: Installation Company Display - Shown before status selection */}
        {mode === 'b' && (
          <section className="bg-gradient-to-r from-blue-600 to-blue-700 rounded-2xl p-6 shadow-lg text-white">
            <div className="flex items-center gap-4">
              <div className="w-16 h-16 bg-white/20 backdrop-blur rounded-xl flex items-center justify-center">
                <Building2 className="w-8 h-8 text-white" />
              </div>
              <div className="flex-1">
                <p className="text-sm text-blue-100 font-medium mb-1">Reviewing Services Provided By</p>
                <p className="text-2xl font-bold">{formData.installerName}</p>
              </div>
        ...
        {/* Section 0: Installation Status (First step for B/C modes) */}
        {mode !== 'public' && (
          <section id="status-section" className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
            <div className="flex items-center gap-2 mb-6 pb-4 border-b border-gray-100">
              <ClipboardList className="w-6 h-6 text-blue-600" />
              <h2 className="text-xl font-bold text-gray-800">Installation Status</h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              <button
                ...
                className={`flex flex-col items-center justify-center p-4 rounded-xl border-2 transition-all duration-200 gap-3 h-32 ${
                  installStatus === 'installed'
                    ? 'border-blue-500 bg-blue-50 ring-1 ring-blue-500'
                    : 'border-gray-200 hover:border-blue-200 hover:bg-gray-50'
                }`}
              >
                <div className={`w-10 h-10 rounded-full flex items-center justify-center ${installStatus === 'installed' ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-500'}`}>
                  <CheckCircle2 className="w-6 h-6" />
                </div>
                <div className="text-center">
                  <span className={`block font-semibold ${installStatus === 'installed' ? 'text-blue-900' : 'text-gray-700'}`}>Installed</span>
              ...
              <button
                ...
                className={`flex flex-col items-center justify-center p-4 rounded-xl border-2 transition-all duration-200 gap-3 h-32 ${
                  installStatus === 'signed'
                    ? 'border-amber-500 bg-amber-50 ring-1 ring-amber-500'
                    : 'border-gray-200 hover:border-amber-200 hover:bg-gray-50'
                }`}
              >
                 <div className={`w-10 h-10 rounded-full flex items-center justify-center ${installStatus === 'signed' ? 'bg-amber-500 text-white' : 'bg-gray-100 text-gray-500'}`}>
                  <HelpCircle className="w-6 h-6" />
                </div>
                <div className="text-center">
                  <span className={`block font-semibold ${installStatus === 'signed' ? 'text-amber-900' : 'text-gray-700'}`}>Signed Contract</span>
                  <span className="text-xs text-gray-500">Awaiting installation</span>
                </div>
              </button>
              ...
              <button
                ...
                className={`flex flex-col items-center justify-center p-4 rounded-xl border-2 transition-all duration-200 gap-3 h-32 ${
                  installStatus === 'considering'
                    ? 'border-amber-500 bg-amber-50 ring-1 ring-amber-500'
                    : 'border-gray-200 hover:border-amber-200 hover:bg-gray-50'
                }`}
              >
                 <div className={`w-10 h-10 rounded-full flex items-center justify-center ${installStatus === 'considering' ? 'bg-amber-500 text-white' : 'bg-gray-100 text-gray-500'}`}>
                  <HelpCircle className="w-6 h-6" />
                </div>
                <div className="text-center">
                  <span className={`block font-semibold ${installStatus === 'considering' ? 'text-amber-900' : 'text-gray-700'}`}>Still Considering</span>
                  <span className="text-xs text-gray-500">Comparing quotes</span>
                </div>
              </button>
              ...
              <button
                ...
                className={`flex flex-col items-center justify-center p-4 rounded-xl border-2 transition-all duration-200 gap-3 h-32 ${
                  installStatus === 'rejected'
                    ? 'border-gray-600 bg-gray-100 ring-1 ring-gray-400'
                    : 'border-gray-200 hover:border-gray-300 hover:bg-gray-50'
                }`}
              >
                 <div className={`w-10 h-10 rounded-full flex items-center justify-center ${installStatus === 'rejected' ? 'bg-gray-600 text-white' : 'bg-gray-100 text-gray-500'}`}>
                  <XCircle className="w-6 h-6" />
                </div>
                <div className="text-center">
                  <span className={`block font-semibold ${installStatus === 'rejected' ? 'text-gray-900' : 'text-gray-700'}`}>Did Not Proceed</span>
                  <span className="text-xs text-gray-500">Chose another / Cancelled</span>
                </div>
              </button>
            </div>

            {/* Context Messages based on selection */}
            {installStatus === 'considering' && (
              <div className="mt-6 p-4 bg-amber-50 text-amber-900 border border-amber-200 rounded-lg flex items-start gap-3 animate-in fade-in slide-in-from-top-1">
                <MessageSquareQuote className="w-5 h-5 mt-0.5 shrink-0" />
                <div>
                  <p className="font-semibold">Reviewing Quote Options</p>
                  <p className="text-sm mt-1">Thank you for your feedback. Your input helps us improve our quoting process.</p>
                </div>
              </div>
            )}
            {installStatus === 'rejected' && (
               <div className="mt-6 p-4 bg-gray-100 text-gray-800 border border-gray-200 rounded-lg flex items-start gap-3 animate-in fade-in slide-in-from-top-1">
                <MessageSquareQuote className="w-5 h-5 mt-0.5 shrink-0" />
                <div>
                  <p className="font-semibold">Decision Recorded</p>
                  <p className="text-sm mt-1">Thank you for letting us know. We appreciate your feedback about your decision.</p>
                </div>
              </div>
            )}
          </section>
        ...
        {/* Main Form Content - Hidden for B/C modes until status is selected */}
        {(mode === 'public' || installStatus) && (
          <div id="main-form-content" className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
            
            {/* Section 1: Select Installer - Hidden for B-side (shown at top), and hidden for C-side considering/rejected states */}
            {mode !== 'b' && !(mode === 'c' && (installStatus === 'considering' || installStatus === 'rejected')) && (
            ...
            {/* Section 3: Your Review - Don't show for considering or rejected states */}
            {installStatus !== 'considering' && installStatus !== 'rejected' && (
              <section className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
                <div className="flex items-center gap-2 mb-6 pb-4 border-b border-gray-100">
                  <Wrench className="w-6 h-6 text-blue-600" />
                  <h2 className="text-xl font-bold text-gray-800">
                    {installStatus === 'signed' ? 'Quote Experience Review' : 
                     'Your Review'}
                  </h2>
                </div>
                  ...
                  <div className="relative">
                    <label className="block font-medium text-gray-700 mb-2">
                      ...
                      <span className="ml-2 text-sm font-normal text-gray-500">
                        {installStatus === 'signed' ? 
                          '(Please share your experience with the sales team, proposal clarity, and installer responsiveness)' :
                    ...
                    <textarea
                      ...
                      placeholder={installStatus === 'signed' ? 
                        'Share your experience with the sales team and proposal process...' :
                        'Share your experience with the installation team...'}
                    />
            ...
            {/* Section 4: System Information - Don't show for considering or rejected states */}
            {(mode === 'b' || !formData.isQuoteOnly) && installStatus !== 'considering' && installStatus !== 'rejected' && (
              <section className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 transition-all duration-300">
                <div className="flex items-center gap-2 mb-6 pb-4 border-b border-gray-100">
                  <Info className="w-6 h-6 text-blue-600" />
                  <h2 className="text-xl font-bold text-gray-800">
                    {formData.isQuoteOnly && mode === 'b' ? 'About your quote' : 'About your system'}
                  </h2>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 animate-in fade-in slide-in-from-top-2 duration-300">
                  ...
                  <div className="md:col-span-2">
                    <label className="block text-sm font-medium text-gray-700 mb-1">
                      {formData.isQuoteOnly && mode === 'b' ? 'Quote Cost' : 'Total System Cost'}
                    </label>
                    <div className="relative">
            ...
            {/* HIDDEN for B-side and C-side (mode === 'b' or 'c') - data is prefilled */}
            {mode === 'public' && (
        ...
        <div className="max-w-4xl mx-auto flex items-center justify-between gap-4">
           <div className="hidden md:block text-sm text-gray-500">
             {Object.keys(errors).length > 0 ? (
                ...
                mode !== 'public' && (!installStatus || installStatus === 'considering' || installStatus === 'rejected') ? "Status recorded. Thank you for your feedback." : "Ensure all required fields are filled before submitting."
             )}
           </div>
           <button
             onClick={handleSubmit}
             disabled={mode !== 'public' && (!installStatus || installStatus === 'considering' || installStatus === 'rejected')}
             className={`w-full md:w-auto md:min-w-[200px] font-bold py-3 px-8 rounded-xl shadow-lg transition-all active:scale-95 ${
               mode !== 'public' && (!installStatus || installStatus === 'considering' || installStatus === 'rejected')
